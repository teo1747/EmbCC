...
**Current position: M0 complete, before M1.** `make && make test` runs green
on the host; the ELF writer skeleton, the predefined-macro table (348 macros,
generated by `tools/gen-predef.sh` from `x86_64-elf-gcc -dM -E`), the test
runner, and an honest `--version` exist. No parser, no codegen — nothing
compiles yet.
